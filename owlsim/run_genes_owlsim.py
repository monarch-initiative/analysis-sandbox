...
def main():
    ...
    disease_dictionary = monarch.process_input_file(args.input)
    ...
    for disease_id, disease in disease_dictionary.items():
        output_file.write("{0}\t{1}\t{2}\t{3}\t{4}\t{5}\t{6}\t{7}\t{8}\t{9}\t{10}\t{11}\t{12}\t{13}\n".format(
                          ...
                          disease["human_gene"], monarch.TAXON_MAP[disease["model_gene_taxon"]],
...
def process_input_file(input_file):
    ...
    with open(input_file, 'rt') as tsvfile:
        ...
        next(reader, None)
        for row in reader:
            (disease, model, model_gene, human_gene) = [monarch.map_iri_to_curie(val) for val in row]
            ...
            human_gene_label = monarch.get_label_from_scigraph(human_gene)
            model_gene_label = monarch.get_label_from_scigraph(model_gene)
